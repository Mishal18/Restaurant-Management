...
exports.getOngoingOrders = async (req, res) => {
    try {
      ...
      const onGoingOrdersGuest = await GuestOrder.find({ paymentStatus : 'pending'}).populate('user','name');
      // map the order ID, total amount, and user name for each ongoing order
      const Guestorder = onGoingOrdersGuest.map(order => ({id: order._id, totalAmount: order.totalAmount, userName: order.user.UserName}))
      ...
      res.render('cashier/printOngoingOrder', { orders ,Guestorder});
    } catch (err) {
      console.error(err);
      res.status(500).send('Server Error');
    }
  ...
  exports.changePaymentStatus = async (req, res) => {
    try {
      ...
      const gorder =await GuestOrder.findById(orderId);
      ...
      gorder.paymentStatus = 'Payment recieved'
