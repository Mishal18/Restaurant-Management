...
exports.postManagerSignupPage = async (req, res) => {
  try {
    ...
    bcrypt.hash(req.body.msignpass, saltrounds, (err, hash) => {
      ...
      manager.save().then(() => {
        req.flash("success", "User added waiting for verification")
...
exports.getAllCategories = async (req, res) => {
  try {
    const categories = await Category.find({}).lean();
    res.render('manager/deletecategory', { categories });
  } catch (error) {
    console.error(error);
    res.status(500).send('Server Error');
  }
...
exports.getItemsDeletePage = async (req, res) => {
  try {
    ...
    res.render('manager/deleteitems', { categories ,items });
  } catch (error) {
    console.error(error);
    res.status(500).send('Server Error');
  }
...
exports.getOngoingOrder = async (req, res) => { 
  try {
    ...
    res.render('manager/OnGoingOrder', { items ,orders ,totalQuantityByProduct});
  } catch (err) {
    console.error(err); 
    res.status(500).send('Server error');
  }
